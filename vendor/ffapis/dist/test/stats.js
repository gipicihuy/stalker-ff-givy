...
async function testStats() {
    ...
    const printStats = (title, data) => {
        ...
        if (data.solostats)
            console.log('Solo:', JSON.stringify(data.solostats, (k, v) => (v && v.value ? v.value : v)));
        if (data.duostats)
            console.log('Duo:', JSON.stringify(data.duostats, (k, v) => (v && v.value ? v.value : v)));
        if (data.quadstats)
            console.log('Squad:', JSON.stringify(data.quadstats, (k, v) => (v && v.value ? v.value : v)));
    ...
    try {
        ...
        console.error('Stats test failed:', (0, types_1.getErrorMessage)(e));
    }
}
testStats();
